...
import { ASAP_LEAD_MIN } from './scheduling'
...
// Flat short-notice surcharges, mirroring Blacklane's tier structure but ~£20 cheaper per band.
const SHORT_NOTICE_TIERS = [
  { maxMin: 120, amount: 90, label: 'Short notice (under 2 hours)' },
  { maxMin: 360, amount: 45, label: 'Short notice (2–6 hours)' },
]
...
function leadMinutes(data, now) {
  if (data.pickupMode === 'now') return ASAP_LEAD_MIN
...
function shortNoticeSurcharge(data, now) {
  const lead = leadMinutes(data, now)
  if (lead == null || lead <= 0) return null
  return SHORT_NOTICE_TIERS.find((t) => lead < t.maxMin) || null
}

function buildRange(total, time, shortNotice) {
  const breakdown = []
  let subtotal = total
  if (isNightTime(time)) {
  ...
  if (shortNotice) {
    subtotal += shortNotice.amount
    breakdown.push({ label: shortNotice.label, amount: shortNotice.amount })
  }
